...
$.validator.addMethod('newmailsendingdatevalidation', function (value, element, params) {
    console.log($("#new-mail-form #NewMail_RegistrationDate").val())
...
function setRegistrationNumber_NewMail() {
    return $.ajax({
        ...
        errorNotification('Erreur', "Erreur lors du génération du N° d'enregistrement.", 3000);
        console.log(response);
    });
}
...
$("#new-mail-form").on("reset", function () {
    ...
    $("#new-mail-file-viewer").attr("src", "/Document Placeholders/FR/SelectDocument_Fr.html");
});
...
$("#new-mail-form").on("submit", function (event) {
    ...
    var createMail_NewMail = $.ajax({
        ...
        $("#new-mail-form")[0].reset();
        $(".new-mail-popup-wrapper").css("display", "none");
        successNotification('Succès', "Courrier ajouté avec succès.", 3000);
        console.log(response);
        return response;
    }).fail(function (response) {
        errorNotification('Erreur', "Erreur lors de l'ajout du courrier.", 3000);
        console.log(response);
    }).always(function () {
        $(".new-mail-popup .processing-icon-block-shown-576").addClass("hidden");
