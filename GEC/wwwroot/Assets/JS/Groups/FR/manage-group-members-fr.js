var contactsDualList = $('#ManageGroupMembers_IdContact').bootstrapDualListbox({
    moveOnSelect: false,
    moveSelectedLabel: 'Ajouter Selectionné',
    moveAllLabel: 'Ajouter Tous',
    removeSelectedLabel: 'Retirer Selectionné',
    removeAllLabel: 'Retirer Tous',
    selectedListLabel: '<strong>Contacts En Groupe</strong>',
    nonSelectedListLabel: '<strong>Contacts Hors Groupe</strong>',
...
function getAndRenderContactsAndGroupMembers_ManageGroupMembers(groupId) {
    ...
    var getGroupMembers_ManageGroupMembers = $.ajax({
        ...
        errorNotification('Erreur', "Erreur lors du chargement des membres du groupe.", 3000);
        console.log(response);
    });
...
$("#manage-group-members-form").on("submit", function (event) {
    ...
    var manageGroupMembers_ManageGroupMembers = $.ajax({
        ...
        resetContactsDualList();
        $(".manage-group-members-popup-wrapper").css("display", "none");
        successNotification('Succès', "Membres du groupe modifiés avec succès.", 3000);
        console.log(response);
        return response;
    }).fail(function (response) {
        errorNotification('Erreur', "Erreur lors du modification des membres du groupe.", 3000);
        console.log(response);
    }).always(function () {
        $(".manage-group-members-popup .processing-icon-block-shown-576").addClass("hidden");
